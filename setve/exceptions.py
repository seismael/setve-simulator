"""Unified Domain Exception Hierarchy and OS Errno Translation Engine for SETVE."""
...
class SetveError(Exception):
    """Base exception for all SETVE simulation and validation errors."""

    @classmethod
    def from_errno(cls, err: OSError, context: str = "") -> SetveError:
        """Map raw OS errno error code to granular domain-specific SetveError subclass."""
...
class AlignmentError(SetveError, ValueError):
...
class PayloadError(SetveError, ValueError):
...
class OrchestratorError(SetveError):
...
class ValidationPlaneError(SetveError):
