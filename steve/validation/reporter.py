...
logger = logging.getLogger("steve.telemetry")
...
class ClusterTelemetrySummary:
    ...
    def to_prometheus_metrics(self) -> str:
        ...
        lines = [
            "# HELP steve_cluster_ops_total Total I/O operations completed by STEVE cluster",
            "# TYPE steve_cluster_ops_total counter",
            f'steve_cluster_ops_total{{run_id="{self.run_id}"}} {self.total_ops}',
            "# HELP steve_cluster_bytes_total Total bytes transferred by STEVE cluster",
            "# TYPE steve_cluster_bytes_total counter",
            f'steve_cluster_bytes_total{{run_id="{self.run_id}"}} {self.total_bytes}',
            "# HELP steve_cluster_throughput_gbps Aggregate cluster throughput in Gbps",
            "# TYPE steve_cluster_throughput_gbps gauge",
            f'steve_cluster_throughput_gbps{{run_id="{self.run_id}"}} '
            f"{self.aggregate_throughput_gbps:.4f}",
            "# HELP steve_cluster_latency_p99_ms Maximum p99 latency observed across workers",
            "# TYPE steve_cluster_latency_p99_ms gauge",
            f'steve_cluster_latency_p99_ms{{run_id="{self.run_id}"}} {self.max_p99_ms:.4f}',
        ]
        if self.divergence is not None:
            valid_int = 1 if self.divergence.is_valid else 0
            lines.extend(
                [
                    "# HELP steve_telemetry_divergence_percent Telemetry skew vs physical probe",
                    "# TYPE steve_telemetry_divergence_percent gauge",
                    f'steve_telemetry_divergence_percent{{run_id="{self.run_id}"}} '
                    f"{self.divergence.divergence_percent:.4f}",
                    "# HELP steve_telemetry_is_valid Boolean flag indicating telemetry validity",
                    "# TYPE steve_telemetry_is_valid gauge",
                    f'steve_telemetry_is_valid{{run_id="{self.run_id}"}} {valid_int}',
                ]
            )
        return "\n".join(lines) + "\n"
    ...
    def format_table(self) -> str:
        ...
        lines = [
            f"+{border}+",
            f"| STEVE SIMULATION & TELEMETRY REPORT: {self.run_id:<42} |",
            f"+{border}+",
